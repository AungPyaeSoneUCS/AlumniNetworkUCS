// file: components/admin/salary-ranges-exporter.tsx
...
type SalaryRangesExporterProps = {
  toggleId: string;
  menuId: string;
  csv: string;
  html: string;
...
export default function SalaryRangesExporter({
  toggleId,
  menuId,
  csv,
  html,
  ...
  useEffect(() => {
    ...
    const buildExcelFile = async () => {
      ...
      try {
        ...
        const ws = wb.addWorksheet("Income Range Report");
        ...
        const summary = [["Total Job Titles", current.totals.positions, "FF0F766E"]];
        summary.forEach((s, i) => {
          const row = 5;
          const col = i * 2 + 1;
        ...
        const head = ["Job Title", "Min Income", "Max Income"];
        head.forEach((h, i) => {
