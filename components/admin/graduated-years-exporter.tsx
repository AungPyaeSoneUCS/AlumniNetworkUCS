// file: components/admin/graduated-years-exporter.tsx
...
type GraduatedYearsExporterProps = {
  toggleId: string;
  menuId: string;
  csv: string;
  html: string;
...
function hexToArgb(hex: string) {
  return "FF" + String(hex ?? "#0f766e").replace("#", "").toUpperCase();
}

export default function GraduatedYearsExporter({
  toggleId,
  menuId,
  csv,
  html,
  ...
  labels,
  degrees,
}: GraduatedYearsExporterProps) {
  const dataRef = useRef({ csv, html, title, pdfLoading, pdfError, graphHtml, rows, totals, labels, degrees });
  dataRef.current = { csv, html, title, pdfLoading, pdfError, graphHtml, rows, totals, labels, degrees };
  ...
  useEffect(() => {
    ...
    const buildExcelFile = async () => {
      ...
      try {
        ...
        const ws = wb.addWorksheet("Graduated Year Report");
        ...
        ws.mergeCells("A5:F5");
        const totalCell = ws.getCell("A5");
        totalCell.value = "Total Graduates: " + String(current.totals.graduates).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        totalCell.font = { size: 12, bold: true, name: "Segoe UI", color: { argb: "FF0F766E" } };
        ...
        const degrees = current.degrees;
        const header = [
          current.labels.no,
          current.labels.years,
          ...degrees,
        ...
        const degreeWidth = Math.max(1, degrees.length);
        for (let i = 1; i <= degreeWidth + 3; i++) ws.getColumn(i).width = 18;

        current.rows.forEach((row, idx) => {
          const r = tableRow + 1 + idx;
          const values = [String(row.no), row.year, ...row.cells.map(String), String(row.total)];
          values.forEach((val, i) => {
            ...
            c.font = {
              size: 11,
              name: "Segoe UI",
              color: { argb: i === 1 ? hexToArgb(row.color) : "FF0F172A" },
              bold: i === 1,
            };
            c.alignment = { vertical: "middle", horizontal: "center" };
          });
