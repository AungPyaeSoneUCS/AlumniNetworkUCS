  ...
  return (
    <div className={`${isDark ? "dark" : ""} h-screen overflow-hidden`}>
      <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300">
        ...
        {/* --- Top Navigation Bar (AdminNav Style) --- */}
        <nav className="flex-none bg-white/90 dark:bg-gray-900/90 backdrop-blur-md z-50 border-b border-gray-100 dark:border-gray-800 shadow-md shadow-gray-200/50 dark:shadow-black/40 transition-all duration-300">
          <div className="max-w-[95rem] mx-auto px-4 h-16 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 whitespace-nowrap flex-shrink-0 group cursor-pointer">
              <div className="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-700 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/30 transform group-hover:scale-105 transition-all duration-300">
                <span className="text-white font-extrabold text-lg drop-shadow-md">U</span>
              </div>
              <span className="font-bold text-base tracking-tight hidden md:inline text-gray-900 dark:text-gray-100">
                {t.portalName}
                {user?.name && <span className="text-blue-500 dark:text-blue-400 font-medium"> ({user.name})</span>}
              </span>
            </div>

            {/* Right: Language, Theme Toggle & Login/Logout */}
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              {status === "authenticated" ? (
                ...
                <Link
                  href="/vote/login"
                  className="text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 px-4 py-1.5 rounded-lg shadow-sm hover:shadow-md transform hover:-translate-y-0.5 transition-all duration-200"
                >
                  {t.loginBtn}
                </Link>
              )}
